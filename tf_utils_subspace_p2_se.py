...
def test_graph(directory, Uk):
    ...
    trn_maskP = tf.placeholder(tf.complex64, shape=(None, args.nrow_GLOB, args.ncol_GLOB, args.n_echo), name='trn_mask')
    loss_maskP = tf.placeholder(tf.complex64, shape=(None, args.nrow_GLOB, args.ncol_GLOB, args.n_echo), name='loss_mask')
    # nw_inputP = tf.placeholder(tf.float32, shape=(None, args.nrow_GLOB, args.ncol_GLOB, args.n_echo, 2), name='nw_input')
    nw_inputP = tf.placeholder(tf.float32, shape=(None, args.nrow_GLOB, args.ncol_GLOB, args.n_basis, 2), name='nw_input')

    nw_output, nw_kspace_output, nw_kspace_output_inv, x0, all_intermediate_outputs, mu = \
               UnrollNet.UnrolledNet(nw_inputP, sens_mapsP, trn_maskP, loss_maskP, UkT_tensor, Uk_invT_tensor).model
    ...
    nw_kspace_output_inv = tf.identity(nw_kspace_output_inv, name='nw_kspace_output_inv')
